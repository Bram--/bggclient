...
fun main() {
    ...
    runBlocking {
        ...
        // Retrieve Scythe and it's comments (12_000+ as of this writing) but limit to only 25 pages.
        val thingsResponse = BggClient.things(arrayOf(169786), comments = true, pageSize = 100).paginate(toPage = lastPage).call()

        // Throw and log error when data is not returned, can also use #isError() or #isSuccess()
        ...
        while (true) {
            ...
            if (currentPage < lastPage) {
                ...
                comments.comments.subList(currentPage * pageSize, min(nextPage * pageSize, comments.comments.size))
                    .forEach {
                        println(commentLine(it))
                    }
