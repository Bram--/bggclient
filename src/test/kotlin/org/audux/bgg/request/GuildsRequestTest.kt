...
class GuildsRequestTest {
    ...
    fun `Makes a request with all parameters`() = runBlocking {
        val engine = TestUtils.setupMockEngine("guilds?id=2310&members=1&page=1")
        BggClient.engine = { engine }
        ...
        assertThat(request.url)
            .isEqualTo(
                Url("https://boardgamegeek.com/xmlapi2/guilds?id=2310&members=1&sort=date&page=1")
            )
        assertThat(response.data?.name).isEqualTo("St Albans Board Games Club")
        assertThat(response.data?.members?.members).hasSize(PaginatedGuilds.PAGE_SIZE)
    }
    ...
    inner class Paginates {
        ...
        fun `Automatically to the end`() = runBlocking {
            ...
            assertThat(engine.requestHistory.map { it.url })
                .containsExactly(
                    Url("https://boardgamegeek.com/xmlapi2/guilds?id=2310&members=1"),
                    Url("https://boardgamegeek.com/xmlapi2/guilds?id=2310&members=1&page=2"),
                    Url("https://boardgamegeek.com/xmlapi2/guilds?id=2310&members=1&page=3"),
                )
            assertThat(response.data?.members?.count).isEqualTo(62)
            assertThat(response.data?.members?.members).hasSize(62)
        }
        ...
        fun `To the toPage parameter`() = runBlocking {
            ...
            assertThat(response.data?.members?.count).isEqualTo(62)
            assertThat(response.data?.members?.members).hasSize(50)
        }
        ...
        fun `From the initial page to the end`() = runBlocking {
            ...
            assertThat(response.data?.members?.count).isEqualTo(62)
            assertThat(response.data?.members?.members).hasSize(37)
        }
        ...
        @Test
        fun `Throws when members param is not set`() {
            runBlocking {
                val engine = TestUtils.setupMockEngine("guilds?id=2310&members=1&page=1")
                BggClient.engine = { engine }
